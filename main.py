...
import infer_mask2former_768 as m2f_768
...
M2F768_CKPT_PATH = "best_model_mask2former (5).pth"
...
async def startup_event():
    ...
    global maskrcnn_model, unet_model, meibomio_model, panoptic_model, panoptic_processor, maskrcnn2_model, maskrcnn3_model, maskrcnn4_model, panoptic6_model, panoptic6_processor, unet2_model, panoptic1024_model, panoptic1024_processor, m2f768_model, m2f768_processor
    try:
        ...
        # Load Mask R-CNN v3 (maskcrnnv3.py — best_model 18)
        try:
            print("Loading Mask R-CNN v3 model...")
            if os.path.exists(MASKRCNN3_CKPT_PATH):
                import torch as _torch
                ...
                _m3.to(device).eval()
                maskrcnn3_model = _m3
                print("✓ Mask R-CNN v3 model loaded successfully")
            else:
                print(f"✗ Mask R-CNN v3 checkpoint not found: {MASKRCNN3_CKPT_PATH}")
        except Exception as e:
            print(f"⚠ Failed to load Mask R-CNN v3 model: {e}")
            maskrcnn3_model = None

        # Load Mask R-CNN v4 (maskcrnnv3 architecture — best_model 20)
        try:
            print("Loading Mask R-CNN v4 model...")
            if os.path.exists(MASKRCNN4_CKPT_PATH):
                import torch as _torch
                ...
                _m4.to(device).eval()
                maskrcnn4_model = _m4
                print("✓ Mask R-CNN v4 model loaded successfully")
            else:
                print(f"✗ Mask R-CNN v4 checkpoint not found: {MASKRCNN4_CKPT_PATH}")
        except Exception as e:
            print(f"⚠ Failed to load Mask R-CNN v4 model: {e}")
            maskrcnn4_model = None

        # Load Panoptic v6 (panoptic6.py — best_model 32, NUM_QUERIES=100)
        try:
            print("Loading Panoptic v6 model...")
            if os.path.exists(PANOPTIC6_CKPT_PATH):
                _p6_model, _p6_proc = panoptic_v6.load_model_server(PANOPTIC6_CKPT_PATH, device)
                panoptic6_model = _p6_model
                panoptic6_processor = _p6_proc
                print("✓ Panoptic v6 model loaded successfully")
            else:
                print(f"✗ Panoptic v6 checkpoint not found: {PANOPTIC6_CKPT_PATH}")
        except Exception as e:
            print(f"⚠ Failed to load Panoptic v6 model: {e}")
            panoptic6_model = None
            panoptic6_processor = None
        ...
        # Load Mask2Former 768 (infer_mask2former_768.py — best_model_mask2former (5), OUTPUT_SIZE=768)
        try:
            print("Loading Mask2Former 768 model...")
            if os.path.exists(M2F768_CKPT_PATH):
                _m768_model, _m768_proc = m2f_768.load_model_server(M2F768_CKPT_PATH, device)
                m2f768_model = _m768_model
                m2f768_processor = _m768_proc
                print("✓ Mask2Former 768 model loaded successfully")
            else:
                print(f"✗ Mask2Former 768 checkpoint not found: {M2F768_CKPT_PATH}")
        except Exception as e:
            print(f"⚠ Failed to load Mask2Former 768 model: {e}")
            m2f768_model = None
            m2f768_processor = None
...
async def health_check():
    ...
    return {
        ...
        "models_loaded": {
            ...
            "mask2former768": m2f768_model is not None,
        },
        "device": str(device),
...
async def model_status():
    ...
    return {
        "models": {
            ...
            "maskrcnn3": {
                "status": "loaded" if maskrcnn3_model is not None else "failed",
                "path": MASKRCNN3_CKPT_PATH,
            },
            "maskrcnn4": {
                "status": "loaded" if maskrcnn4_model is not None else "failed",
                "path": MASKRCNN4_CKPT_PATH,
            },
            "panoptic6": {
            ...
            "mask2former768": {
                "status": "loaded" if m2f768_model is not None else "failed",
                "path": M2F768_CKPT_PATH,
            },
        },
        "device": str(device),
...
async def analyze_maskrcnn2(
    ...
    try:
        ...
        # Resolve contour: user-drawn mask OR UNet fallback (same as panoptic endpoint)
        ...
        if contour_mask is not None and contour_mask.filename:
            ...
            unet_mask_img = Image.fromarray((tarsus_bin * 255).astype(np.uint8))
            with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as ctmp:
                unet_mask_img.save(ctmp.name)
                contour_tmp_path = ctmp.name
...
async def analyze_maskrcnn3(
    ...
    """Mask R-CNN v3 — best_model (18).pth"""
    if maskrcnn3_model is None:
        raise HTTPException(
            status_code=503,
            detail={"error": "Mask R-CNN v3 not loaded", "message": f"Checkpoint '{MASKRCNN3_CKPT_PATH}' missing or failed."}
        )
    ...
    try:
        ...
        pred_instance = maskrcnn_v3.infer_with_model(
            image_path=temp_file_path,
            model=maskrcnn3_model,
            device=device,
            contour_path=contour_tmp_path,
        )
        ...
        return jsonable_encoder({
            "success": True,
            "message": "Mask R-CNN v3 analysis completed successfully",
            "data": {
                "processed_image": f"data:image/png;base64,{img_base64}",
                "avg_tortuosity": round(tortuosity_data['avg_tortuosity'], 3),
                "num_glands": tortuosity_data['num_glands'],
        ...
        import traceback
        tb = traceback.format_exc()
        print(f"[MASKRCNN3] ERROR: {e}\n{tb}")
        raise HTTPException(status_code=500, detail=f"Mask R-CNN v3 analysis failed: {str(e)}")


@app.post("/analyze-maskrcnn4")
async def analyze_maskrcnn4(
    ...
    """Mask R-CNN v4 — best_model (20).pth (same arch as v3)"""
    if maskrcnn4_model is None:
        raise HTTPException(
            status_code=503,
            detail={"error": "Mask R-CNN v4 not loaded", "message": f"Checkpoint '{MASKRCNN4_CKPT_PATH}' missing or failed."}
        )
    ...
    try:
        ...
        return jsonable_encoder({
            "success": True,
            "message": "Mask R-CNN v4 analysis completed successfully",
            "data": {
                "processed_image": f"data:image/png;base64,{img_base64}",
                "avg_tortuosity": round(tortuosity_data['avg_tortuosity'], 3),
                "num_glands": tortuosity_data['num_glands'],
        ...
        import traceback
        tb = traceback.format_exc()
        print(f"[MASKRCNN4] ERROR: {e}\n{tb}")
        raise HTTPException(status_code=500, detail=f"Mask R-CNN v4 analysis failed: {str(e)}")


@app.post("/analyze-panoptic6")
async def analyze_panoptic6(
    ...
    """Panoptic v6 — Mask2Former NUM_QUERIES=100, best_model (32).pth"""
    if panoptic6_model is None or panoptic6_processor is None:
        raise HTTPException(status_code=503, detail={"error": "Panoptic v6 not loaded", "message": f"Checkpoint '{PANOPTIC6_CKPT_PATH}' missing or failed."})
    ...
    try:
        ...
        pred_instance = panoptic_v6.infer_with_model(
            image_path=temp_file_path,
            model=panoptic6_model,
            processor=panoptic6_processor,
            device=device,
            contour_path=contour_tmp_path,
        )
        ...
        return jsonable_encoder({
            "success": True,
            "message": "Panoptic v6 analysis completed successfully",
            "data": {
                "processed_image": f"data:image/png;base64,{img_base64}",
                "avg_tortuosity": round(tortuosity_data['avg_tortuosity'], 3),
                "num_glands": tortuosity_data['num_glands'],
        ...
        import traceback
        tb = traceback.format_exc()
        print(f"[PANOPTIC6] ERROR: {e}\n{tb}")
        raise HTTPException(status_code=500, detail=f"Panoptic v6 analysis failed: {str(e)}")
...
@app.post("/analyze-mask2former768")
async def analyze_mask2former768(
    file: UploadFile = File(...),
    um_per_px: float = Form(1.0),
    tarsus_version: str = Form("v1"),
    contour_mask: Optional[UploadFile] = File(None),
    ...
    """Mask2Former 768 — 768×768, 150 queries (best_model_mask2former (5).pth)"""
    if m2f768_model is None or m2f768_processor is None:
        raise HTTPException(status_code=503, detail={"error": "Mask2Former 768 not loaded", "message": f"Checkpoint '{M2F768_CKPT_PATH}' missing or failed."})
    ...
    try:
        ...
        pred_instance = m2f_768.infer_with_model(
            image_path=temp_file_path,
            model=m2f768_model,
            processor=m2f768_processor,
            device=device,
            contour_path=contour_tmp_path,
        )
        ...
        return jsonable_encoder({
            "success": True,
            "message": "Mask2Former 768 analysis completed successfully",
            "data": {
                "processed_image": f"data:image/png;base64,{img_base64}",
                "avg_tortuosity": round(tortuosity_data['avg_tortuosity'], 3),
                "num_glands": tortuosity_data['num_glands'],
        ...
        import traceback
        tb = traceback.format_exc()
        print(f"[M2F768] ERROR: {e}\n{tb}")
        raise HTTPException(status_code=500, detail=f"Mask2Former 768 analysis failed: {str(e)}")
